    ...
    <>
      <nav className="fixed top-0 left-0 w-full z-[100] border-b border-white/5 bg-resin-bg/50 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5 flex justify-between items-center">
          <NavLink to="/" className="flex items-center gap-3 font-bold text-xl tracking-tight hover:opacity-80 transition-opacity">
            <div className="w-6 h-6 bg-gradient-to-br from-resin-cyan to-resin-purple rounded transform rotate-45 shadow-[0_0_20px_rgba(59,130,246,0.5)]" />
            UNIPRO
          </NavLink>
        ...
        <div className="relative h-full max-w-7xl mx-auto px-8 md:px-20 flex flex-col justify-center">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32">
                ...
                {navLinks.map((link, index) => (
                  ...
                  >
                    <div className="flex items-center gap-6">
                      ...
                      <span className={`text-2xl font-semibold tracking-tight ${location.pathname === link.path ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
                        {link.name}
                      </span>
                    </div>
            ...
            <div className="flex flex-col justify-center space-y-12">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {!isLoggedIn ? (
                    <>
                      <button 
                        onClick={toggleAuth}
                        className="flex items-center justify-center gap-3 px-8 py-5 glass-card rounded-[24px] hover:bg-white/10 transition-all text-sm font-medium text-white active:scale-95"
                      >
                        <LogIn size={18} className="text-resin-cyan" /> Sign In
                      </button>
                      <button 
                        onClick={toggleAuth}
                        className="flex items-center justify-center gap-3 px-8 py-5 bg-white text-black rounded-[24px] hover:bg-resin-cyan transition-all text-sm font-semibold active:scale-95"
                      >
                    ...
                    <button 
                      onClick={toggleAuth}
                      className="col-span-full flex items-center justify-center gap-3 px-8 py-5 glass-card rounded-[24px] hover:bg-red-500/10 border-red-500/20 hover:border-red-500/40 transition-all text-sm font-medium text-red-400 active:scale-95"
                    >
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  <div className="flex-1">
                    <h4 className="font-semibold text-lg group-hover:text-resin-cyan transition-colors">
              ...
              <div className="pt-8 flex gap-8 text-sm text-gray-600">
